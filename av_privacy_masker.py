...
class AVPrivacyMasker:
    ...
    def fast_pixelate(self, bgr, mask, block=16, noise=20):
        H, W = mask.shape
        # Mosaic whole image once (super fast), then copy only masked pixels
        small = cv2.resize(bgr, (max(1, W//block), max(1, H//block)), interpolation=cv2.INTER_LINEAR)
        pix   = cv2.resize(small, (W, H), interpolation=cv2.INTER_NEAREST)
        if noise > 0:
            n = np.random.randint(-noise, noise+1, bgr.shape, dtype=np.int16)
            tmp = pix.astype(np.int16) + n
            pix = np.clip(tmp, 0, 255).astype(np.uint8)
        out = bgr.copy()
        m = mask.astype(bool)
        out[m] = pix[m]
        return out
